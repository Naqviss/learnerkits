...
export default async function Home({params}:{params:Promise<{locale:string}>}){
  ...
  const popularSimulations=visibleSubjectSlugs.flatMap((slug)=>{
    const subject=getLocalizedSubject(raw,slug);
    return subject.simulations.filter((sim)=>sim.featured).map((sim)=>({...sim,subjectSlug:slug,subjectName:getLocalizedSubjectName(raw,slug)}));
  });
  return <main className="educationHome">
    ...
    <section className="container section educationPopular"><div className="sectionHeading"><div><div className="eyebrow">{c.library.priority}</div><h2>{c.library.priorityTitle}</h2></div><p>{c.library.priorityBody}</p></div><div className="priorityStrip">{popularSimulations.map(sim=><Link href={`/${raw}/simulations/${sim.slug}`} className="priorityStripCard" key={sim.slug}><span>{sim.subjectName}</span><strong>{sim.title}</strong></Link>)}</div></section>
