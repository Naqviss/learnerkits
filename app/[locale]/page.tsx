...
const subjectGlyphs = { space: "✦", physics: "↗", geography: "◒", biology: "⌁", chemistry: "⚗", mathematics: "∑" } as const;
  ...
  return <main className="educationHome">
    ...
    <section className="container educationHero">
      <div className="educationHeroCopy">
        ...
        <div className="learningMeta"><span>{c.home.grades}</span><span>{c.home.labCount}</span><span>{c.home.noSignup}</span></div>
      </div>
    ...
    <section className="container subjectGateway educationSubjectGateway"><div className="sectionHeading"><div><div className="eyebrow">{c.home.subjects}</div><h2>{c.home.choose}</h2></div><p>{c.home.chooseBody}</p></div><div className="subjectGatewayGrid">{subjectSlugs.map(slug=>{const data=getLocalizedSubject(raw,slug);return <Link className={`subjectGatewayCard educationSubjectCard subject-${slug}`} key={slug} href={`/${raw}/subjects/${slug}`}><div className="subjectCardTop"><span className="gatewayGlyph">{subjectGlyphs[slug]}</span><span className="gradeTag">{data.gradeBand}</span></div><div><small>{data.simulations.length} {c.home.interactiveLabs}</small><h3>{getLocalizedSubjectName(raw,slug)}</h3><p>{data.learningObjectives[0]}</p></div><div className="subjectCardFooter"><span>{data.concepts.slice(0,2).join(" · ")}</span><b>{c.home.viewSubject} →</b></div></Link>})}</div></section>
