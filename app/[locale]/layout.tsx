...
export async function generateMetadata({ params }: { params: Promise<{locale:string}> }): Promise<Metadata> { const { locale: raw } = await params; const locale = isLocale(raw) ? raw : "en"; const m = getMessages(locale),adsense=adsenseClientId(); return { title: { default: m.meta.title, template: `%s · ${m.meta.title}` }, description: m.meta.description, icons: { icon: "/favicon.svg", shortcut: "/favicon.svg", apple: "/learnerkits-mark.svg" }, verification: { google: "9SDKLsHd0vlmWO8Hl0c71834IrwGrBKIz_wTrxpm1Xk" },other:adsense?{"google-adsense-account":adsense}:undefined }; }
