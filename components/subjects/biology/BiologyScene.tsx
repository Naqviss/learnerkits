...
export function BiologyScene({slug,values,time,heart,onSelect}:{slug:string;values:Values;time:number;heart:number;onSelect?:(index:number)=>void}){
 ...
 useEffect(()=>{
  ...
  const profile=renderProfile(loadSettings().quality);renderer.setPixelRatio(Math.min(devicePixelRatio,profile.pixelRatioCap));renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.08;container.appendChild(renderer.domElement);
  const scene=new THREE.Scene(),camera=new THREE.PerspectiveCamera(43,1,.1,100);camera.position.set(9,6,12);const controls=new OrbitControls(camera,renderer.domElement);controls.enablePan=false;controls.enableZoom=false;controls.enableDamping=true;controls.target.set(0,0,0);controls.update();controls.saveState();orbit.current=controls;
  scene.add(new THREE.HemisphereLight(0xe4edff,0x493f50,2.1));const key=new THREE.DirectionalLight(0xffffff,3);key.position.set(-5,9,7);scene.add(key);const rim=new THREE.DirectionalLight(0x7ea8ef,1.5);rim.position.set(7,2,-6);scene.add(rim);
  ...
  function tube(points:THREE.Vector3[],radius:number,color:number,parent:THREE.Object3D=scene){const curve=new THREE.CatmullRomCurve3(points),o=mesh(new THREE.TubeGeometry(curve,48,radius,10,false),material(color),parent);return o;}
  ...
  if(slug==="animal-cell-3d"){
   ...
   camera.position.set(8,4,13);const heartGroup=new THREE.Group();scene.add(heartGroup);heartGroup.rotation.y=-.2;
   const ra=sphere(1.35,0x557eb7,new THREE.Vector3(-1.55,1.25,0),heartGroup),rv=sphere(1.7,0x5079ad,new THREE.Vector3(-1.05,-1.2,.1),heartGroup),la=sphere(1.3,0xc56d7d,new THREE.Vector3(1.4,1.2,0),heartGroup),lv=sphere(1.8,0xc45d70,new THREE.Vector3(1,-1.2,0),heartGroup);
   ra.scale.set(.8,1,.9);rv.scale.set(.85,1,.9);la.scale.set(.8,1,.9);lv.scale.set(.85,1,.9);
   tube([new THREE.Vector3(-1.7,4,0),new THREE.Vector3(-1.7,2.2,0)],.28,0x638cc1,heartGroup);tube([new THREE.Vector3(-1,-2.2,0),new THREE.Vector3(-2.8,-3.6,0),new THREE.Vector3(-4,-2.7,0)],.25,0x6289b8,heartGroup);
   tube([new THREE.Vector3(4,-2.7,0),new THREE.Vector3(2.8,-3.6,0),new THREE.Vector3(1.1,-2.25,0)],.25,0xc56677,heartGroup);tube([new THREE.Vector3(1.1,-2,0),new THREE.Vector3(2.7,2.8,0),new THREE.Vector3(1.3,4,0)],.32,0xc45d70,heartGroup);
   const routePositions=[new THREE.Vector3(4,-2.7,0),new THREE.Vector3(-1.7,4,0),new THREE.Vector3(-1.55,1.25,1.4),new THREE.Vector3(-1.4,0,1.4),new THREE.Vector3(-1.05,-1.2,1.7),new THREE.Vector3(-1.9,-2.3,1.1),new THREE.Vector3(-4,-2.7,0),new THREE.Vector3(4,-2.7,0),new THREE.Vector3(1.4,1.2,1.4),new THREE.Vector3(1.2,0,1.5),new THREE.Vector3(1,-1.2,1.8),new THREE.Vector3(1.8,1.3,1.2),new THREE.Vector3(4,-2.7,0)];
   const blood=sphere(.22,0xeac1c8,routePositions[0],heartGroup);const label=sphere(.06,0xffffff,new THREE.Vector3());
   update=()=>{const i=Math.min(heartRoute.length-1,live.current.heart),p=routePositions[i];blood.position.lerp(p,.12);const oxygenated=i>=7||i===0||i===12;(blood.material as THREE.MeshStandardMaterial).color.setHex(oxygenated?0xf07c8e:0x76a4df);const pulse=1+Math.sin(performance.now()*.006)*.035;[ra,rv,la,lv].forEach(o=>o.scale.multiplyScalar(pulse/o.scale.y));label.visible=false;};
  }
  ...
  return()=>{cancelAnimationFrame(raf);ro.disconnect();renderer.domElement.removeEventListener("pointerup",click);controls.dispose();orbit.current=null;geos.forEach(g=>g.dispose());mats.forEach(m=>m.dispose());renderer.dispose();renderer.domElement.remove();};
 },[slug]);
 return <div className="bioThree"><div ref={host} className="bioCanvas" role="img" aria-label={`${slug.replaceAll("-"," ")} interactive 3D model`}/>{failed&&<p className="bioFallback">3D is unavailable. You can still complete the activity with the controls and measurements.</p>}<div className="bioOrbit"><span>Drag to inspect · structures enlarged for learning</span><button onClick={()=>orbit.current?.reset()}>Reset view</button></div></div>;
}
